...
func (e *Identify2WithUID) runIdentifyUI(m libkb.MetaContext) (err error) {
	...
	if e.arg.IdentifyBehavior.ShouldSuppressTrackerPopups() {
		// xxx holy shit this is used by saltpack etc. switched on ShouldSuppressTrackerPopups???
	...
	if outcome.IsOK() {
		// xxx this might not be the right thing to use. It might be too strict. We don't want to force users to track to get stellar to work.
...
func (e *Identify2WithUID) maybeNotify(mctx libkb.MetaContext, explanation string) {
	...
	if target.IsNil() {
		mctx.CDebugf("xxx Identify2WithUID.maybeNotify(%v, %v) nope missing UID", target, explanation)
		return
	}
	if e.arg.IdentifyBehavior.WarningInsteadOfErrorOnBrokenTracks() {
		mctx.CDebugf("xxx Identify2WithUID.maybeNotify(%v, %v) nope WarningInsteadOfErrorOnBrokenTracks", target, explanation)
		return
	}
	mctx.CDebugf("xxx Identify2WithUID.maybeNotify(%v, %v) -> sending", target, explanation)
	go libkb.NotifyIdentifyHeedTrackingSuccess(mctx, target)
}
